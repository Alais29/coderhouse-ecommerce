...
export const isValidProduct = (producto: IItem): boolean | Error => {
  ...
  if (!isUrl(producto.foto)) {
    throw new Error('Verifica los datos, la url de la foto no es válida');
  }

  if (!isValidCode(producto.codigo)) {
    throw new Error('Verifica los datos, el código ingresado no es válido');
  }

  if (isNaN(producto.stock)) {
    throw new Error('Verifica los datos, el stock debe ser un número');
  }

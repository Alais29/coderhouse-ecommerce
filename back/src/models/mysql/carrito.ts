...
export class CarritosModelMySql {
      ...
      if (!exists) {
        this.connection.schema
          .createTable('carrito', (carritoTable) => {
            ...
            carritoTable.timestamp('timestamp').defaultTo(this.connection.fn.now());
  ...
  async get(id?: string): Promise<IItem | IItem[]> {
    try {
      if (id) {
        const producto = await this.connection('carrito').where('id', Number(id));
        return producto[0];
      }
  ...
  async save(id: string): Promise<IItem> {
    try {
      ...
      if (productToAddInCart) {
        ...
        const productToAdd = await this.connection('productos').where('id', Number(id));
        if (productToAdd.length) {
          const productAddedId = await this.connection('carrito').insert(productToAdd[0]);
  ...
  async delete(id: string): Promise<IItem[]> {
    try {
      const productDeleted = await this.connection('carrito').where('id', Number(id)).del();
      if (!productDeleted) {
        throw new NotFound('El producto que desea eliminar no está en el carrito');
      } else {

...
const ProductList = ({ handleToggleShowModal, productos, location, handleRemove, handleAddToCart }: IProductList) => {
  return (
    <>
      <div className={cx(styles['product-list'], 'my-4', 'd-flex', 'flex-wrap', 'justify-content-center', {'flex-column': location === 'cart'})}>
        ...
        {!isEmpty(productos) && location === 'cart' && handleRemove && productos.map((producto: IItemAPI) => (
          <ProductCarrito
            key={producto.id}
            product={producto}
